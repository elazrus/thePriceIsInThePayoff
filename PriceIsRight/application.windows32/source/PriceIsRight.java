...
public class PriceIsRight extends PApplet {
...
public int WHITE = color(255);
public int ORANGE = color(250, 162, 27);
public int DARK_ORANGE = color(161, 95, 34);
public int RED = color(237, 53, 35);
public int YELLOW = color(255, 221, 0);
public int BLUE = color(79, 144, 195);
public int LIGHT_BLUE = color(115, 168, 193);
...
public void drawCircles(float r, float x, float y) {
  ellipseMode(CENTER);
  for (int i=0; i < 10; i++) {
    pushMatrix();
    translate(x, y);
...
public float getTopX() {
  ...
  if (mouseX > graphX+graphWidth+22) topX = width-32.1f;
  return topX;
}
...
public float getTopY() {
  ...
  if (mouseX > graphX+graphWidth+22) {
    ...
    topY = 23.9f+getQuadrant(quadHeight)*quadHeight;
  }
  return topY;
}

public float getQuadrant(float quadHeight) {
...
public class About {
  ...
  public void render() {
    ...
    text(s, textX, textY + 1.5f*textYOffset);
    ...
    text(s, textX, textY + 2.5f*textYOffset);
  }
}
public class Button {
  float x;
...
public class ButtonStrip {
  ...
  public ButtonStrip(GameData gameData, float x, float y, float w, float h) {
    ...
    for (int i=0; i < buttons.length; i++) {
      buttons[i] = new Button(x+w/2, (i+0.35f)*h/10, new String[2], i, height/10);
    }
    buttons[0].s[0] = "about";
    buttons[0].s[1] = "";
    buttons[1].s[0] = "help";
...
public class GameGraph {
  ...
  public void getCoords() {  
    for (int i=0; i < numGameTicks; i++) {
      ...
      dataFontSizes.add(12.0f);
      dataRadii.add(5.0f);
      dataLineWeights.add(1.0f);
  ...
  public void update() {
    ...
    for (int i=0; i < points.size(); i++) {
      ...
      if ((dx*dx + dy*dy) <= 25/*dx < gameGraph.wBtwTicks/2*/) {
        ...
        dataFontSizes.set(i, 40.0f);
        dataRadii.set(i, 8.0f);
        dataLineWeights.set(i, 3.0f);
        ...
        dataFonts.set(i, font_bold);  
        dataFontSizes.set(i, 40.0f);
        dataRadii.set(i, 12.0f);
        dataLineWeights.set(i, 2.0f);
        ...
        dataFontSizes.set(i, 12.0f);
        dataFonts.set(i, font_normal);
        dataRadii.set(i, 5.0f);
        dataLineWeights.set(i, 1.0f);
      }
    }
...
public class Help {
  ...
  public void render() {
    ...
    stroke(BLUE);
    strokeWeight(3);
    drawBracketOpenRight(graphX+graphWidth+50, 150, 80, 3.05f*height/4);
...
public class TickVert extends Tick {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "PriceIsRight" };
    if (passedArgs != null) {
